/*=====================================================================
   GemiOS CLOUD HYPERVISOR - v51.1.0-ALPHA (DRIVER REFORM UPDATE)
   Modular Architecture, Driver Manager, Focus Shields, Sticky Note 2.0.
=====================================================================*/
...
if (window.__GEMIOS_BOOTED__) {
    ...
    (() => {
        ...
        class DriverManager {
            constructor() {
                this.drivers = {
                    audio: localStorage.getItem('GemiOS_Driver_Audio') !== 'false',
                    anim: localStorage.getItem('GemiOS_Driver_Anim') !== 'false',
                    theme: localStorage.getItem('GemiOS_Theme') || 'dark',
                    accent: localStorage.getItem('GemiOS_Accent') || '#0078d7'
                };
            }
            init() {
                this.update('theme', this.drivers.theme);
                this.update('accent', this.drivers.accent);
            }
            update(type, val) {
                ...
                if(type === 'theme') {
                    localStorage.setItem('GemiOS_Theme', val);
                    document.documentElement.dataset.theme = val;
                    if(val === 'light') document.body.classList.add('light-mode');
                    else document.body.classList.remove('light-mode');
                }
                if(type === 'accent') {
                    localStorage.setItem('GemiOS_Accent', val);
        ...
        class WindowManager {
            ...
            _createWindow({pid, app, fileData}) {
                ...
                this.focus(wid); // Auto focus new window
                this.audio.play('open'); this._renderDock(); 
            }
            focus(wid){ 
                // Activate shields for all windows behind
                document.querySelectorAll('.win').forEach(w => {
                    let shield = w.querySelector('.focus-shield');
                    if(shield) shield.style.display = 'block';
                    w.style.boxShadow = "0 20px 50px rgba(0,0,0,0.5), inset 0 1px 1px rgba(255,255,255,0.2)";
                });
                // Disable shield and bring forward target window
                const el = document.getElementById(wid); 
                if (el) { 
                    el.style.zIndex = ++this.zIndex; 
                    el.style.boxShadow = "0 25px 60px rgba(0,0,0,0.8), 0 0 0 1px var(--accent)"; 
                    let shield = el.querySelector('.focus-shield');
        ...
        // FALLBACK CORE APPS (Includes the new Settings app referencing DriverManager)
        window.GemiCoreApps = {
            'sys_term': { id: 'sys_term', tag: 'sys', icon: '💻', title: 'Terminal', width: 500, html: (pid) => `<div id="t-out-${pid}" style="flex-grow:1; background:#0a0a0a; color:#38ef7d; padding:10px; font-family:monospace; overflow-y:auto; border-radius:6px; box-shadow:inset 0 0 10px rgba(0,0,0,0.8);">GemiOS v51.1.0-ALPHA Shell.<br>Type 'help' for commands.</div><div style="display:flex; background:#111; padding:8px; border-radius:6px; margin-top:5px; border:1px solid #333;"><span id="t-path-${pid}" style="color:#0078d7; margin-right:8px; font-weight:bold;">C:/Users/${GemiOS.user}></span><input type="text" id="t-in-${pid}" style="flex-grow:1; background:transparent; color:#38ef7d; border:none; outline:none; font-family:monospace; font-size:14px;" onkeydown="GemiOS.handleTerm(event, ${pid}, this)"></div>`, onLaunch: (pid) => { let p = `C:/Users/${GemiOS.user}`; GemiOS.termStates[pid] = p; document.getElementById(`t-path-${pid}`).innerText = p + '>'; setTimeout(()=>document.getElementById('t-in-'+pid).focus(),100); } },
        ...
        class Core {
            ...
            async init(){
                this.drivers.init(); // Initialize Driver Manager
                this.injectStyles(); 
                this._buildUI(); 
            ...
            login() {
                ...
                if(!localStorage.getItem('GemiOS_V51_1_ALPHA_Celebrated')) {
                    setTimeout(() => {
                        this.bus.emit('notify', {title: "🚀 V51.1 ALPHA DRIVERS", msg: "Focus Shields & Sticky Note 2.0 active.", success: true});
                        localStorage.setItem('GemiOS_V51_1_ALPHA_Celebrated', 'true');
                    }, 1000);
                }
            }
            ...
            injectStyles() {
                ...
                s.textContent = `
                    ...
                    #sticky-header { font-weight:bold; margin-bottom:5px; cursor:grab; padding-bottom:5px; border-bottom:1px solid rgba(0,0,0,0.1); }
            ...
            _buildUI() {
                let currentVer = localStorage.getItem('GemiOS_Cache_Ver') || '51.1.0-ALPHA';
            ...
            stickyHeader.onmousedown = (e) => {
                ...
                let offsetX = e.clientX - stickyNote.offsetLeft;
                let offsetY = e.clientY - stickyNote.offsetTop;
                const move = ev => { 
                    stickyNote.style.left = ev.clientX - offsetX + 'px'; 
                    stickyNote.style.top = Math.max(0, ev.clientY - offsetY) + 'px'; 
                    stickyNote.style.right = 'auto'; // Disable right anchoring
                };
                const up = () => { 
        ...
        async createSnapshot(auto = false) {
            let k = localStorage.getItem('GemiOS_Cache_Kernel') || ""; let r = localStorage.getItem('GemiOS_Cache_Registry') || ""; let v = localStorage.getItem('GemiOS_Cache_Ver') || "51.1.0-ALPHA";
