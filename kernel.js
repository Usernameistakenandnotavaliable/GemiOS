// =========================================================================
// GemiOS CLOUD HYPERVISOR - v30.1.0 (THE PERSISTENCE UPDATE)
// =========================================================================
...
if (bootVersion === 'v1') { document.open(); document.write(atob('')); document.close(); }
    ...
    class VirtualFileSystem {
        constructor() {
            ...
            if(!drive) {
                this.root = {
                    "C:": {
                        "System": { "boot.log": "GemiOS V30.1 Initialized.", "sys_mail.json": "[]" },
        ...
        format() { localStorage.removeItem('GemiOS_TreeFS'); sessionStorage.removeItem('GemiOS_Session'); localStorage.removeItem('GemiOS_Wall'); localStorage.removeItem('GemiOS_Accent'); localStorage.removeItem('GemiOS_Cache_Kernel'); localStorage.removeItem('GemiOS_Cache_Ver'); location.reload(); }
    ...
    const AppRegistry = {
        ...
        'sys_update': { icon: '☁️', title: 'Cloud Updater', width: 380, html: () => `<div class="sys-card" style="text-align:center; flex-grow:1;"><div style="font-size:40px;">☁️</div><h3 style="margin:5px 0;">GitHub Update Center</h3><p style="font-size:13px; opacity:0.8;">Kernel: <b id="kern-ver">v30.1.0-PERSISTENCE</b></p><div id="upd-stat" style="font-size:12px; min-height:15px;"></div><button id="upd-btn" onclick="GemiOS.triggerOTA(this)" class="btn-primary" style="margin-top:10px;">Check for Updates</button></div>` },
        'sys_log': { icon: '📋', title: 'Chronicles', width: 500, html: () => `<div style="flex-grow:1; overflow-y: auto; padding-right: 5px;"><div class="sys-card" style="border-left:4px solid #38ef7d;"><b>v30.1.0 (Persistence Update)</b> - True Air-Gapped NVRAM Locking. The OS no longer requires internet after the first boot. Fully functional local OTA Flasher.</div><div class="sys-card"><b>v30.0.1 (Complete Ecosystem)</b> - Restored all 30+ applications to the App Registry.</div><div class="sys-card"><b>v30.0.0 (The Release)</b> - GemiPhone, GPM Manager, GemiMail, MegaStore.</div><div class="sys-card"><b>v29.0.0 (Creator)</b> - GemiMaker Block Engine.</div><div class="sys-card"><b>v28.0.0 (Megapatch)</b> - GemiAssist Voice, GemiDAW, P2P Link.</div><div class="sys-card"><b>v1.0 (Legacy)</b> - The True Original.</div></div>` },
        ...
        'app_weather': { icon: '🌤️', title: 'GemiWeather', width: 400, html: (pid) => `<div style="display:flex; flex-direction:column; flex-grow:1;"><canvas id="weather-cvs-${pid}" style="width:100%; flex-grow:1; background:linear-gradient(to bottom, #1a2a6c, #112240);"></canvas></div>`, onLaunch: (pid) => { setTimeout(() => { let cvs = document.getElementById(`weather-cvs-${pid}`); if(!cvs) return; let ctx = cvs.getContext('2d'); let particles = []; for(let i=0; i<100; i++) particles.push({x:Math.random()*1000, y:Math.random()*1000, s:Math.random()*3+1}); GemiOS.weatherItvs = GemiOS.weatherItvs || {}; GemiOS.weatherItvs[pid] = setInterval(() => { if(!document.getElementById(`weather-cvs-${pid}`)) { clearInterval(GemiOS.weatherItvs[pid]); return; } cvs.width = cvs.offsetWidth; cvs.height = cvs.offsetHeight; ctx.clearRect(0,0,cvs.width,cvs.height); ctx.fillStyle = 'rgba(255,255,255,0.6)'; particles.forEach(p => { ctx.fillRect(p.x, p.y, 2, p.s*3); p.y += p.s*5; if(p.y > cvs.height) { p.y = -10; p.x = Math.random()*cvs.width; } }); }, 30); }, 100); }, onKill: (pid) => clearInterval(GemiOS.weatherItvs[pid]) }
    };
    ...
    class CoreOS {
            ...
            for(let file in desk) {
                if(file.endsWith('.app') || file.endsWith('.gbs') || file.endsWith('.gzip')) {
                    ...
                    if(file.endsWith('.app')) {
                        let appId = desk[file]; let app = this.Registry[appId] || STORE_CATALOG[file];
        ...
        exportNVRAM() {
            ...
            let a = document.createElement('a'); a.href = url; a.download = `GemiOS_Backup_${this.user}.gemos`;
            document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url);
            this.notify("GemiSync", "NVRAM Snapshot Exported Successfully!");
        }

        // --- TRUE OTA FIRMWARE FLASHER ---
        async triggerOTA(btn) {
            ...
            try {
                ...
                let currentVer = localStorage.getItem('GemiOS_Cache_Ver') || "30.0.1-PATCH";
                ...
                if (d.version !== currentVer) {
                    ...
                    btn.onclick = async () => {
                        ...
                        let fill = document.getElementById('ota-fill'); let text = document.getElementById('ota-text');
                        
                        try {
                            text.innerText = "Downloading Kernel payload from GitHub...";
                            fill.style.width = "40%";
                            let kRes = await fetch("https://raw.githubusercontent.com/Usernameistakenandnotavaliable/GemiOS/main/kernel.js" + cb);
                            if(!kRes.ok) throw new Error("Kernel download failed.");
                            let kCode = await kRes.text();
                            
                            text.innerText = "Writing to NVRAM...";
                            fill.style.width = "80%";
                            
                            // PHYSICALLY FLASH THE NVRAM
                            ...
                            text.innerText = "UPDATE FAILED: " + e.message;
                    ...
                    <div id="start-menu">
                        <div class="start-header">
                            ...
                            <div>
                                ...
                                <div style="font-size:12px; opacity:0.7; font-family:monospace;">GemiOS 30.1.0 / <span style="color:var(--accent); font-weight:bold;">PERSISTENCE</span></div>
                            </div>
                        </div>
                    ...
                    <div id="taskbar-container">
                        <div id="taskbar">
                            ...
                            <div style="display:flex; align-items:center; gap:15px; margin-left:10px; padding-left:15px; border-left:1px solid rgba(255,255,255,0.1);">
                                ...
                                <div style="font-weight:600; font-size:12px; background:rgba(255, 255, 255, 0.2); color:white; padding:4px 10px; border-radius:20px; border:1px solid rgba(255,255,255,0.4);">v30.1 AIR-GAPPED</div>
