// =========================================================================
// GemiOS CLOUD HYPERVISOR - v39.0.0 (THE ENTERPRISE UPDATE)
// =========================================================================

const bootVersion = localStorage.getItem('GemiOS_TargetVersion') || 'v39';
console.log("[BOOT] Hypervisor targeting state: " + bootVersion);

if (bootVersion !== 'v39' && bootVersion !== 'v38') { 
    document.open(); document.write('<body style="background:#000;color:#0f0;font-family:monospace;padding:50px;"><h2>Legacy OS Archived.</h2><button onclick="localStorage.setItem(\'GemiOS_TargetVersion\',\'v39\');location.reload()" style="padding:10px;background:#0f0;color:#000;font-weight:bold;cursor:pointer;border:none;">Reboot to V39</button></body>'); document.close(); 
} else {
    class VirtualFileSystem {
        constructor() {
            ...
            if(!drive) {
                this.root = { "C:": { "System": { "boot.log": "GemiOS V39.0 Initialized.", "sys_mail.json": "[]" }, "Users": { "Admin": { "Documents": {}, "Pictures": {}, "Desktop": {}, "Downloads": {} }, "Guest": { "Documents": {}, "Pictures": {}, "Desktop": {}, "Downloads": {} } } } };
                this.forceSave();
    ...
    const AppRegistry = {
        ...
        'sys_update': { tag: 'sys', icon: '☁️', title: 'Cloud Updater', width: 380, html: () => `<div class="sys-card" style="text-align:center; flex-grow:1;"><div style="font-size:40px;">☁️</div><h3 style="margin:5px 0;">Dual-OTA Updater</h3><p style="font-size:13px; opacity:0.8;">Kernel: <b id="kern-ver">v39.0.0-ENTERPRISE</b></p><div id="upd-stat" style="font-size:12px; min-height:15px; margin-bottom:10px;">Ready to scan Cloud Network.</div><button id="upd-btn" onclick="GemiOS.triggerOTA(this)" class="btn-primary">Check for Updates</button></div>` },
        'sys_log': { tag: 'sys', icon: '📋', title: 'Chronicles', width: 500, html: () => `<div style="flex-grow:1; overflow-y: auto; padding-right: 5px;"><div class="sys-card" style="border-left:4px solid #38ef7d;"><b>v39.0.0 (Enterprise)</b> - OS Installer Deployed (Home/Pro/Edu). Local App Registry Fusion active. GemiDev Studio launched. Developer Economy running. Improved Shutdown synthesis.</div><div class="sys-card"><b>v38.1.0 (Linker Hotfix)</b> - Repaired Dynamic Linker ID mapping to prevent execution errors on custom apps.</div><div class="sys-card"><b>v38.0.0 (Command & Control)</b> - Added GemiSearch and Native Action Center. Context menus enhanced.</div><div class="sys-card"><b>v37.0.0 (Economy)</b> - Virtual Wallet added. Monetized App Store.</div><div class="sys-card"><b>v36.1.0 (Dynamic Linker)</b> - Kernel and Registry decoupled. Dual-OTA flasher active.</div></div>` },
    ...
    class CoreOS {
        constructor() { 
            ...
            this.notifications = []; 
            this.edition = localStorage.getItem('GemiOS_Edition'); // V39 EDITIONS
        }
                ...
                <div style="position:fixed; top:0; left:0; width:100vw; height:100vh; background:linear-gradient(135deg, #0f2027, #203a43, #2c5364); color:white; display:flex; flex-direction:column; justify-content:center; align-items:center; z-index:9999999; font-family:'Inter', sans-serif;">
                    ...
                    <div style="display:flex; gap:20px;">
                        <div onclick="GemiOS.installOS('Home')" style="background:rgba(255,255,255,0.05); border:1px solid rgba(255,255,255,0.1); padding:30px; border-radius:12px; width:200px; text-align:center; cursor:pointer; transition:0.3s;" onmouseover="this.style.background='rgba(255,255,255,0.1)'; this.style.transform='translateY(-5px)'" onmouseout="this.style.background='rgba(255,255,255,0.05)'; this.style.transform='translateY(0)'">
                            ...
                            <p style="font-size:12px; color:#888; margin:0;">Standard web applications and media tools.</p>
                        </div>
                        <div onclick="GemiOS.installOS('Pro')" style="background:rgba(255,255,255,0.05); border:1px solid rgba(255,255,255,0.1); padding:30px; border-radius:12px; width:200px; text-align:center; cursor:pointer; transition:0.3s;" onmouseover="this.style.background='rgba(255,255,255,0.1)'; this.style.transform='translateY(-5px)'; this.style.borderColor='var(--accent)'" onmouseout="this.style.background='rgba(255,255,255,0.05)'; this.style.transform='translateY(0)'; this.style.borderColor='rgba(255,255,255,0.1)'">
                            ...
                            <p style="font-size:12px; color:#888; margin:0;">Unlocks Developer Studio and advanced tools.</p>
                        </div>
                        <div onclick="GemiOS.installOS('Education')" style="background:rgba(255,255,255,0.05); border:1px solid rgba(255,255,255,0.1); padding:30px; border-radius:12px; width:200px; text-align:center; cursor:pointer; transition:0.3s;" onmouseover="this.style.background='rgba(255,255,255,0.1)'; this.style.transform='translateY(-5px)'; this.style.borderColor='#38ef7d'" onmouseout="this.style.background='rgba(255,255,255,0.05)'; this.style.transform='translateY(0)'; this.style.borderColor='rgba(255,255,255,0.1)'">
                            ...
                            <p style="font-size:12px; color:#888; margin:0;">15% permanent discount on all Educational apps.</p>
                        </div>
                    </div>
                </div>
            `;
        ...
        async loadDynamicRegistry() {
            ...
            let customApps = JSON.parse(localStorage.getItem('GemiOS_CustomApps') || '{}'); // V39 LOCAL REGISTRY
            
            let fuse = () => {
                // 1. Load Core Cloud Apps
                for(let file in window.GemiRegistry) {
                    let app = window.GemiRegistry[file];
                    if(app && app.id) this.Registry[app.id] = app; 
                }
                // 2. Load Local Dev Apps
                for(let cFile in customApps) {
                    let cApp = customApps[cFile];
                    let evalHtml = (pid) => { return cApp.htmlString; }; // Reconstruct function from string
        ...
        startOTADaemon() { 
            ...
            setInterval(async () => { 
                ...
                try { 
                    ...
                    if(r.ok) { 
                        let d = await r.json(); let currentVer = localStorage.getItem('GemiOS_Cache_Ver') || "39.0.0-ENTERPRISE"; 
        ...
        // V39 CINEMATIC SHUTDOWN (Lydian Pad Fade)
        playShutdownChime() { 
            ...
            const freqs = [130.81, 261.63, 329.63, 370.00, 392.00]; // C Lydian voicing
            freqs.forEach((f) => { 
                let osc = this.actx.createOscillator(); let gain = this.actx.createGain(); 
                osc.type = 'triangle'; osc.frequency.value = f; osc.connect(gain); gain.connect(this.actx.destination); 
                osc.start(t); gain.gain.setValueAtTime(0, t); gain.gain.linearRampToValueAtTime(0.15, t + 0.5); gain.gain.exponentialRampToValueAtTime(0.0001, t + 4.0); osc.stop(t + 4.0); 
            }); 
        }
        ...
        publishApp(pid) {
            ...
            this.loadDynamicRegistry(); // Refresh registry
            this.notify("GemiDev Studio", `${title} published to Store!`, true);
            this.playNote(800);
        }

        // --- THE GEMICRYPT HOTFIX METHODS (Secure Scope) ---
        tradeCrypt(action, pid) {
            let prcEl = document.getElementById(`crypt-prc-${pid}`);
            if(!prcEl) return; // App is closed
            let cost = Math.floor(this.cryptPrice || 100);
            
            if(action === 'buy') {
                if(this.wallet >= cost) {
                    this.wallet -= cost; this.cryptShares = (this.cryptShares || 0) + 1;
                    this.saveWallet(); localStorage.setItem('GemiOS_CryptShares', this.cryptShares); this.updateWalletUI();
                    document.getElementById(`crypt-shares-${pid}`).innerText = this.cryptShares;
                    document.getElementById(`crypt-wallet-${pid}`).innerText = '🪙 ' + Math.floor(this.wallet);
                    this.playNote(800);
                } else { this.notify("Trade Failed", "Insufficient funds.", false); this.playNote(150); }
            } else {
                if(this.cryptShares > 0) {
                    ...
                    document.getElementById(`crypt-wallet-${pid}`).innerText = '🪙 ' + Math.floor(this.wallet);
                    this.playNote(1200);
                } else { this.notify("Trade Failed", "No shares to sell.", false); this.playNote(150); }
            }
        }
        ...
        renderStore(pid) { 
            ...
            for(let f in window.GemiRegistry) { 
                ...
                // V39 Education Discount Logic
                let price = a.price || 0;
                if(this.edition === 'Education' && a.tag === 'edu') price = Math.floor(price * 0.85);
                ...
                let tagHtml = (this.edition === 'Education' && a.tag === 'edu') ? '<span style="background:#38ef7d; color:black; padding:2px 5px; border-radius:3px; font-size:9px; margin-left:5px;">EDU DISCOUNT</span>' : '';
        ...
        async triggerOTA(btn) {
            ...
            try {
                ...
                let currentVer = localStorage.getItem('GemiOS_Cache_Ver') || "39.0.0-ENTERPRISE";
                    ...
                    <div id="start-menu">
                        <div class="start-header">
                            ...
                            <div>
                                ...
                                <div style="font-size:12px; opacity:0.7; font-family:monospace;">GemiOS 39.0 / <span style="color:var(--accent); font-weight:bold;">${this.edition.toUpperCase()}</span></div>
                            </div>
                        </div>
                    ...
                    <div id="taskbar-container">
                        <div id="taskbar">
                            ...
                            <div style="display:flex; align-items:center; gap:15px; margin-left:10px; padding-left:15px; border-left:1px solid rgba(255,255,255,0.1);">
                                ...
                                <div style="font-weight:600; font-size:12px; background:rgba(255, 255, 255, 0.2); color:white; padding:4px 10px; border-radius:20px; border:1px solid rgba(255,255,255,0.4);">v39.0</div>
