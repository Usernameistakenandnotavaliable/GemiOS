...
(() => {
  ...
  class ProcessManager {
    ...
    async launch(appId){ 
        ...
        // V50.1 FIX: Removed the redundant click sound that caused the "beep" before the open chime!
  ...
  class WindowManager {
    ...
    _createWindow({pid, app}) {
      ...
      this.audio.play('open'); // The single correct sound
      this._renderDock();
    }
  ...
  // CORE APPS: Simplified Emojis to fix the missing font box `[]` bug!
  window.GemiCoreApps = {
      ...
      // V50.1: Settings now includes the Auto-Backup Hub!
      ...
      // V50.1 Updater connects directly to Auto-Backup
  ...
  class Core {
    constructor(){
      ...
      this.bus.on('notify', ({title,msg,success})=>{ 
          ...
          this.audio.play(success?'success':'error');
      });
    }
      ...
      if(localStorage.getItem('GemiOS_AutoBackup') === null) {
          setTimeout(() => {
              if(confirm("GemiOS v50.1 features NVRAM Snapshots. Would you like to enable Automatic Backups before System Updates? (You can change this later in Settings)")) {
                  ...
                  localStorage.setItem('GemiOS_AutoBackup', 'false');
              }
          }, 3000);
      }
    }
    ...
    injectStyles() {
        ...
        // Enforce universal emoji font fallbacks
        s.textContent = `
            ...
            .dock-icon:hover { background:rgba(255,255,255,0.15); transform:translateY(-5px); border-color:rgba(255,255,255,0.3); }
            ...
            #desktop-icons { display:grid; grid-template-columns: repeat(auto-fill, 90px); gap: 10px; padding: 20px; pointer-events:none; position:absolute; top:0; left:0; width:100%; height:100%; z-index:10; align-content:start;}
    ...
    _buildUI() {
        ...
        <div id="start-menu">
            <div class="start-header"><div style="font-size:35px;">👑</div><div><div style="font-size:20px;font-weight:600;">Admin</div><div style="font-size:12px;opacity:0.7;">GemiOS 50.1 / PRO</div></div></div>
            <div id="start-menu-items" style="padding:10px; max-height:400px; overflow-y:auto;">
                <div class="start-cat" style="font-size:11px; color:#888; margin:10px;">SYSTEM APPS</div>
                ...
                <div class="start-cat" style="font-size:11px; color:#888; margin:10px; margin-top:20px;">CREATOR STUDIO</div>
    ...
    async triggerOTA(btn) {
        ...
        try {
            ...
            if (!r.ok) throw new Error(); let d = await r.json();
            let currentVer = localStorage.getItem('GemiOS_Cache_Ver') || "50.1.0-BIOS";
            if (d.version !== currentVer) {
                ...
                btn.onclick = async () => {
                    // Auto-Backup execution before flashing
    ...
    async renderStore(pid) { 
        ...
        for(let f in window.GemiRegistry) { 
            ...
            let bId = `st-btn-${a.id}-${pid}`; let price = a.price || 0;
    ...
    async buyApp(filename, appId, pid, btnId, price) { 
        ...
        if(await this.VFS.write('C:/Users/Admin/Desktop', filename, appId)) { this.bus.emit('notify', {title:"Success", msg:`Downloaded ${filename}!`}); this.renderDesktopIcons(); let btn = document.getElementById(btnId); if(btn) { btn.className = 'btn-sec'; btn.innerText = 'Installed'; btn.disabled = true; } } 
    }
  }
