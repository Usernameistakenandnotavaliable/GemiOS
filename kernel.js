...
if (window.__GEMIOS_BOOTED__) {
    ...
    (() => {
        ...
        /* --- OS CORE SYSTEMS --- */
        class EventBus { constructor() { this.handlers = new Map(); } on(ev, fn) { if (!this.handlers.has(ev)) this.handlers.set(ev, []); this.handlers.get(ev).push(fn); } emit(ev, data) { const arr = this.handlers.get(ev); if (!arr) return; arr.forEach(fn => fn(data)); } }
    ...
    class Core {
        ...
        login() {
            ...
            if(!localStorage.getItem('GemiOS_V50_2_Celebrated')) {
                setTimeout(() => {
                    this.bus.emit('notify', {title: "🏆 V50.2 MONOLITH", msg: "Zero Dependencies. 100% Native.", success: true});
