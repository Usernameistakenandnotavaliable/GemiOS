// =========================================================================
// GemiOS CLOUD HYPERVISOR - v37.0.0 (THE ECONOMY UPDATE)
// =========================================================================

const bootVersion = localStorage.getItem('GemiOS_TargetVersion') || 'v37';
console.log("[BOOT] Hypervisor targeting state: " + bootVersion);

if (bootVersion !== 'v37' && bootVersion !== 'v36') { 
    document.open(); document.write('<body style="background:#000;color:#0f0;font-family:monospace;padding:50px;"><h2>Legacy OS Archived.</h2><button onclick="localStorage.setItem(\'GemiOS_TargetVersion\',\'v37\');location.reload()" style="padding:10px;background:#0f0;color:#000;font-weight:bold;cursor:pointer;border:none;">Reboot to V37.0</button></body>'); document.close(); 
} else {
    ...
    const AppRegistry = {
        ...
        'sys_update': { icon: '☁️', title: 'Cloud Updater', width: 380, html: () => `<div class="sys-card" style="text-align:center; flex-grow:1;"><div style="font-size:40px;">☁️</div><h3 style="margin:5px 0;">Dual-OTA Updater</h3><p style="font-size:13px; opacity:0.8;">Kernel: <b id="kern-ver">v37.0.0-ECONOMY</b></p><div id="upd-stat" style="font-size:12px; min-height:15px; margin-bottom:10px;">Ready to scan Cloud Network.</div><button id="upd-btn" onclick="GemiOS.triggerOTA(this)" class="btn-primary">Check for Updates</button></div>` },
        'sys_log': { icon: '📋', title: 'Chronicles', width: 500, html: () => `<div style="flex-grow:1; overflow-y: auto; padding-right: 5px;"><div class="sys-card" style="border-left:4px solid #38ef7d;"><b>v37.0.0 (The Economy Update)</b> - Virtual Wallet added. Monetized App Store. Fully functional GemiCrypt Exchange. Ethereal Audio Synthesis strings.</div><div class="sys-card"><b>v36.1.0 (Dynamic Linker)</b> - Kernel and Registry decoupled. Dual-OTA flasher active.</div><div class="sys-card"><b>v35.0.0 (The BIOS)</b> - Added F2 Pre-Boot setup utility.</div><div class="sys-card"><b>v34.0.0 (Persistent Drive)</b> - 10MB Quota & Safe Wipe.</div><div class="sys-card"><b>v1.0 (Legacy)</b> - The Original.</div></div>` },
    ...
    class CoreOS {
        constructor() { 
            ...
            this.wallet = parseInt(localStorage.getItem('GemiOS_Wallet')) || 500; // THE ECONOMY WALLET
        }
        ...
        async loadDynamicRegistry() {
            ...
            if(regCache) {
                try { eval(regCache); Object.assign(this.Registry, window.GemiRegistry); } 
                ...
                try {
                    ...
                    if(r.ok) {
                        let code = await r.text(); localStorage.setItem('GemiOS_Cache_Registry', code);
                        eval(code); Object.assign(this.Registry, window.GemiRegistry);
                    }
        ...
        startOTADaemon() { 
            ...
            setInterval(async () => { 
                ...
                try { 
                    ...
                    if(r.ok) { 
                        let d = await r.json(); let currentVer = localStorage.getItem('GemiOS_Cache_Ver') || "37.0.0-ECONOMY"; 
        ...
        playStartupChime() { 
            ...
            const freqs = [261.63, 329.63, 392.00, 493.88, 587.33]; // CMaj9 Sweep
            freqs.forEach((f, i) => { 
                ...
                gain.gain.exponentialRampToValueAtTime(0.0001, t + i * 0.1 + 3.0); 
                osc.stop(t + i * 0.1 + 3.0); 
            }); 
        }
        
        playShutdownChime() { 
            if(!this.actx) this.actx = new (window.AudioContext || window.webkitAudioContext)(); if(this.actx.state === 'suspended') this.actx.resume(); const t = this.actx.currentTime; 
            const freqs = [523.25, 415.30, 311.13, 261.63]; // C minor sweep down
            freqs.forEach((f, i) => { 
                let osc = this.actx.createOscillator(); let gain = this.actx.createGain(); 
                osc.type = 'triangle'; osc.frequency.value = f; osc.connect(gain); gain.connect(this.actx.destination); 
                osc.start(t + i * 0.15); 
                gain.gain.setValueAtTime(0, t + i * 0.15); 
                gain.gain.linearRampToValueAtTime(0.2, t + i * 0.15 + 0.1); 
                gain.gain.exponentialRampToValueAtTime(0.0001, t + i * 0.15 + 1.5); 
        ...
        buyApp(filename, appId, pid, btnId, price) { 
            if(this.wallet < price) {
                this.notify("Transaction Failed", `Insufficient funds. Needs 🪙 ${price}`, false);
                this.playNote(150);
                return;
            }
                ...
                if(base === 'help') { out.innerHTML += '<br>cmds: ls, cd, mkdir, echo, cat, rm, clear, gpm install [app.app]'; } else if(base === 'clear') { out.innerHTML = ''; }
                    ...
                    if(args[1] === 'install' && args[2]) { 
                        ...
                        if(window.GemiRegistry && window.GemiRegistry[appName]) { 
                            ...
                            if(this.wallet >= price) {
                                this.wallet -= price; this.saveWallet(); this.updateWalletUI();
                                out.innerHTML += `<br>[GPM] Processing transaction... -🪙${price}`; 
                                if(this.VFS.write('C:/Users/' + this.user + '/Desktop', appName, window.GemiRegistry[appName].id)) { 
        ...
        exportFile(path, filename) {
            ...
            this.notify("GemiShare", `${filename} exported to Host OS.`);
        }
                    ...
                    <div id="start-menu">
                        <div class="start-header">
                            ...
                            <div>
                                ...
                                <div style="font-size:12px; opacity:0.7; font-family:monospace;">GemiOS 37.0 / <span style="color:var(--accent); font-weight:bold;">ECONOMY</span></div>
                            </div>
                        </div>
                    ...
                    <div id="taskbar-container">
                        <div id="taskbar">
                            ...
                            <div style="display:flex; align-items:center; gap:15px; margin-left:10px; padding-left:15px; border-left:1px solid rgba(255,255,255,0.1);">
                                ...
                                <div style="font-weight:600; font-size:12px; background:rgba(255, 255, 255, 0.2); color:white; padding:4px 10px; border-radius:20px; border:1px solid rgba(255,255,255,0.4);">v37.0 ECONOMY</div>
