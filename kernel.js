// =========================================================================
// GemiOS CLOUD HYPERVISOR - v45.0.0 (THE ZERO-TRUST UPDATE)
// =========================================================================

const bootVersion = localStorage.getItem('GemiOS_TargetVersion') || 'v45';
console.log("[BOOT] Hypervisor targeting state: " + bootVersion);

if (bootVersion !== 'v45' && bootVersion !== 'v44') { 
    document.open(); document.write('<body style="background:#000;color:#0f0;font-family:monospace;padding:50px;"><h2>Legacy OS Archived.</h2><button onclick="localStorage.setItem(\'GemiOS_TargetVersion\',\'v45\');location.reload()" style="padding:10px;background:#0f0;color:#000;font-weight:bold;cursor:pointer;border:none;">Reboot to V45</button></body>'); document.close(); 
} else {
    class VirtualFileSystem {
        constructor() {
            ...
            if(!drive) {
                this.root = { "C:": { "System": { "boot.log": "GemiOS V45.0 Initialized.", "sys_mail.json": "[]" }, "Users": { "Admin": { "Documents": {}, "Pictures": {}, "Desktop": {}, "Downloads": {} }, "Guest": { "Documents": {}, "Pictures": {}, "Desktop": {}, "Downloads": {} } } } };
                this.forceSave();
    ...
    class WindowManager {
        ...
        createWindow(pid, title, content, width) {
            let wid = 'win_' + pid;
            let watermark = `<div style="position:absolute; bottom:4px; right:8px; font-size:9px; color:inherit; opacity:0.3; pointer-events:none; font-weight:bold; font-family:sans-serif; z-index:9999;">© 2026 GemiOS Security</div>`;
            let html = `
    ...
    class ProcessManager {
        ...
        launch(appId, fileData = null) {
            ...
            this.processes[pid] = { id: appId, title: app.title, raw: app }; // Expose raw app to AV Memory Scanner
    ...
    const AppRegistry = {
        ...
        'sys_update': { tag: 'sys', icon: '☁️', title: 'Cloud Updater', width: 380, html: () => `<div class="sys-card" style="text-align:center; flex-grow:1;"><div style="font-size:40px;">☁️</div><h3 style="margin:5px 0;">Dual-OTA Updater</h3><p style="font-size:13px; opacity:0.8;">Kernel: <b id="kern-ver">v45.0.0-ZEROTRUST</b></p><div id="upd-stat" style="font-size:12px; min-height:15px; margin-bottom:10px;">Ready to scan Cloud Network.</div><button id="upd-btn" onclick="GemiOS.triggerOTA(this)" class="btn-primary">Check for Updates</button></div>` },
        'sys_log': { tag: 'sys', icon: '📋', title: 'Chronicles', width: 500, html: () => `<div style="flex-grow:1; overflow-y: auto; padding-right: 5px;"><div class="sys-card" style="border-left:4px solid #38ef7d;"><b>v45.0.0 (Zero-Trust)</b> - Massive GemiDefender update (Active Memory Scanning & Process Killing). CoinDrainer malware discovered in the wild.</div><div class="sys-card"><b>v44.1.0 (Heuristic Patch)</b> - Re-engineered Antivirus to perform active deep-string checks on Sideloads.</div><div class="sys-card"><b>v44.0.0 (Multimedia)</b> - V44 Expanded System Sound Profiles. Added PixelPro and GemiBeat.</div><div class="sys-card"><b>v43.0.0 (Global Network)</b> - Deployed simulated Global Server.</div></div>` },
    ...
    class CoreOS {
        ...
        startOTADaemon() { 
            ...
            setInterval(async () => { 
                ...
                try { 
                    ...
                    if(r.ok) { 
                        let d = await r.json(); let currentVer = localStorage.getItem('GemiOS_Cache_Ver') || "45.0.0-ZEROTRUST"; 
        ...
        // V45 ZERO-TRUST CARTRIDGE LOGIC
        importCartridge(b64String) {
            try {
                ...
                // HEURISTIC FIREWALL INTERCEPT
                let payload = appData.htmlString;
                if(payload.includes('localStorage.clear') || payload.includes('VFS.format') || payload.includes('GemiOS.wallet -=') || payload.includes('wallet = 0')) {
                    this.notify("THREAT BLOCKED", "GemiDefender blocked a Malicious Cartridge Payload!", false);
                    this.playSysSound('error');
                    return; // Abort installation entirely
                }
        ...
        async triggerOTA(btn) {
            ...
            try {
                ...
                let currentVer = localStorage.getItem('GemiOS_Cache_Ver') || "45.0.0-ZEROTRUST";
                    ...
                    <div id="start-menu">
                        <div class="start-header">
                            ...
                            <div>
                                ...
                                <div style="font-size:12px; opacity:0.7; font-family:monospace;">GemiOS 45.0 / <span style="color:var(--accent); font-weight:bold;">${this.edition.toUpperCase()}</span></div>
                            </div>
                        </div>
                    ...
                    <div id="taskbar-container">
                        <div id="taskbar">
                            ...
                            <div style="display:flex; align-items:center; gap:15px; margin-left:10px; padding-left:15px; border-left:1px solid rgba(255,255,255,0.1);">
                                ...
                                <div style="font-weight:600; font-size:12px; background:rgba(255, 255, 255, 0.2); color:white; padding:4px 10px; border-radius:20px; border:1px solid rgba(255,255,255,0.4);">v45.0</div>
