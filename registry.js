// =========================================================================
// GEMIOS APP REGISTRY & STORE CATALOG - v45.0 (THE ZERO-TRUST UPDATE)
// =========================================================================

window.GemiRegistry = {
    // === V45.0 THE DEFENEDER & THE VIRUS ===
    'GemiDefender.app': { 
        price: 300, tag: 'pro', id: 'app_defend', icon: '🛡️', desc: 'Active Memory Scanner & AV.', title: 'GemiDefender Ultimate', width: 650, 
        html: (pid) => `
            <div style="background:#111; color:white; padding:15px; border-radius:6px; flex-grow:1; display:flex; flex-direction:column;">
                <div style="display:flex; align-items:center; gap:15px; margin-bottom:20px; border-bottom:1px solid #333; padding-bottom:15px;">
                    ...
                    <div><h2 style="margin:0; color:#38ef7d;" id="av-status-${pid}">Zero-Trust Active</h2><p style="margin:0; font-size:12px; color:#aaa;">GemiOS Security Operations</p></div>
                </div>
                <div style="display:flex; gap:10px; margin-bottom:15px;">
                    <button onclick="GemiOS.Registry['app_defend'].scanMem(${pid})" class="btn-primary" id="av-btn-mem-${pid}" style="flex:1;">🧠 Scan Active Memory</button>
                    <button onclick="GemiOS.Registry['app_defend'].scanNVRAM(${pid})" class="btn-sec" id="av-btn-nvram-${pid}" style="flex:1; margin:0;">💽 Deep Scan NVRAM</button>
                </div>
                <div id="av-log-${pid}" style="flex-grow:1; background:#000; border:1px solid #333; border-radius:4px; padding:10px; font-family:monospace; font-size:12px; overflow-y:auto; max-height:200px;">GemiDefender Engine Ready...<br>Awaiting manual sweep command.</div>
            </div>`, 
        scanMem: (pid) => {
            let btn = document.getElementById(`av-btn-mem-${pid}`); 
            let log = document.getElementById(`av-log-${pid}`); 
            let icon = document.getElementById(`av-icon-${pid}`); 
            let status = document.getElementById(`av-status-${pid}`); 
            
            btn.disabled = true; log.innerHTML += "<br>> Initializing RAM Sweep...<br>"; 
            icon.innerText = "🧠"; status.innerText = "Analyzing PIDs..."; status.style.color = "#ffb400";
            
            let threats = [];
            let procs = GemiOS.PM.processes;
            
            setTimeout(() => {
                for(let procId in procs) {
                    let proc = procs[procId];
                    log.innerHTML += `Scanning PID [${procId}] : ${proc.title}...<br>`;
                    
                    // The Heuristic Memory Check
                    let htmlCode = proc.raw && proc.raw.htmlString ? proc.raw.htmlString : (proc.raw && proc.raw.html ? proc.raw.html(procId) : "");
                    if(htmlCode.includes("GemiOS.wallet -=") || htmlCode.includes("GemiOS.wallet = 0") || proc.id === 'app_virus') {
                        threats.push(procId);
                        log.innerHTML += `<span style="color:#ff4d4d;">[!] ROGUE PROCESS DETECTED: ${proc.title} [PID: ${procId}]</span><br>`;
                    }
                }
                
                if(threats.length > 0) {
                    icon.innerText = "⚠️"; status.innerText = "Memory Compromised!"; status.style.color = "#ff4d4d"; 
                    log.innerHTML += `<span style="color:#ff4d4d; font-weight:bold;">Found ${threats.length} malicious process(es).</span><br>`;
                    btn.innerText = "KILL ROGUE PROCESSES"; btn.style.background = "#ff4d4d"; btn.disabled = false;
                    GemiOS.playSysSound('error');
                    
                    btn.onclick = () => {
                        threats.forEach(tPid => {
                            GemiOS.PM.kill(tPid);
                            log.innerHTML += `<span style="color:#38ef7d;">Assassinated PID: ${tPid}</span><br>`;
                        });
                        GemiOS.playSysSound('success');
                        icon.innerText = "🛡️"; status.innerText = "Zero-Trust Active"; status.style.color = "#38ef7d"; 
                        btn.innerText = "🧠 Scan Active Memory"; btn.style.background = "var(--accent)"; btn.onclick = () => GemiOS.Registry['app_defend'].scanMem(pid);
                        GemiOS.notify("GemiDefender", "Rogue processes terminated.", true);
                    };
                } else {
                    icon.innerText = "🛡️"; status.innerText = "Zero-Trust Active"; status.style.color = "#38ef7d"; 
                    log.innerHTML += `<span style="color:#38ef7d; font-weight:bold;">Memory is clean. No unauthorized hooks.</span><br>`;
                    btn.disabled = false; GemiOS.playSysSound('success');
                }
                log.scrollTop = log.scrollHeight;
            }, 1000);
        },
        scanNVRAM: (pid) => { 
            let btn = document.getElementById(`av-btn-nvram-${pid}`); 
            let log = document.getElementById(`av-log-${pid}`); 
            let icon = document.getElementById(`av-icon-${pid}`); 
            let status = document.getElementById(`av-status-${pid}`); 
            
            btn.disabled = true; log.innerHTML += "<br>> Initializing VFS Deep Scan...<br>"; 
            icon.innerText = "💽"; status.innerText = "Scanning Disk..."; status.style.color = "#ffb400"; 
            
            let desk = GemiOS.VFS.getDir('C:/Users/' + GemiOS.user + '/Desktop'); 
            let threats = []; let signatures = ['localStorage.clear', 'VFS.format', 'VFS.delete', 'GemiOS.wallet -='];
            
            setTimeout(() => { 
                for(let file in desk) { 
                    log.innerHTML += `Scanning ${file}...<br>`; 
                    let appId = desk[file]; let appData = GemiOS.Registry[appId];
                    if (appData) {
                        let payload = appData.htmlString ? appData.htmlString : (appData.html ? appData.html(9999).toString() : "");
                        let isThreat = false;
                        for(let sig of signatures) { if(payload.includes(sig)) isThreat = true; }
                        if(appId === 'app_virus' || isThreat) { threats.push(file); log.innerHTML += `<span style="color:#ff4d4d;">[!] HEURISTIC ALERT: Malicious Payload detected in ${file}</span><br>`; } 
                    }
                } 
                
                if(threats.length > 0) { 
                    icon.innerText = "⚠️"; status.innerText = "NVRAM at Risk!"; status.style.color = "#ff4d4d"; 
                    log.innerHTML += `<span style="color:#ff4d4d; font-weight:bold;">Found ${threats.length} threat(s).</span><br>`; 
                    btn.innerText = "Quarantine Files"; btn.style.background = "#ff4d4d"; btn.disabled = false; 
                    GemiOS.playSysSound('error');
                    btn.onclick = () => { 
                        threats.forEach(t => { GemiOS.VFS.delete('C:/Users/' + GemiOS.user + '/Desktop', t); log.innerHTML += `<span style="color:#38ef7d;">Quarantined: ${t}</span><br>`; }); 
                        GemiOS.renderDesktopIcons(); GemiOS.playSysSound('success'); 
                        icon.innerText = "🛡️"; status.innerText = "Zero-Trust Active"; status.style.color = "#38ef7d"; 
                        btn.innerText = "💽 Deep Scan NVRAM"; btn.style.background = "rgba(255,255,255,0.1)"; btn.onclick = () => GemiOS.Registry['app_defend'].scanNVRAM(pid); 
                        GemiOS.notify("GemiDefender", "All threats neutralized.", true); 
                    }; 
                } else { 
                    icon.innerText = "🛡️"; status.innerText = "Zero-Trust Active"; status.style.color = "#38ef7d"; 
                    log.innerHTML += `<span style="color:#38ef7d; font-weight:bold;">No file threats found. NVRAM secure.</span><br>`; 
                    btn.disabled = false; GemiOS.playSysSound('success'); 
                } 
                log.scrollTop = log.scrollHeight; 
            }, 1500); 
    ...
    'GemiVirus.app': { 
        price: 0, tag: 'sys', id: 'app_virus', icon: '💰', desc: 'Generates free passive income for your wallet!', title: 'Free GemiCoins', width: 400, 
        html: (pid) => `
            <div style="text-align:center; padding:30px; display:flex; flex-direction:column; align-items:center; background:#111; border-radius:6px;">
                <h2 style="color:#38ef7d; margin-top:0;">Mining GemiCoins...</h2>
                <div class="spinner" style="margin:20px 0; border-top-color:#38ef7d;" id="v-spin-${pid}"></div>
                <p id="virus-msg-${pid}" style="font-family:monospace; color:#38ef7d;">Connecting to crypto pool...</p>
            </div>`, 
        onLaunch: (pid) => { 
            setTimeout(() => { 
                let msg = document.getElementById(`virus-msg-${pid}`); let spin = document.getElementById(`v-spin-${pid}`);
                if(msg) { msg.innerText = "Hooking into Wallet API..."; msg.style.color = "#ffb400"; spin.style.borderTopColor = "#ffb400"; } 
            }, 2000); 

            setTimeout(() => { 
                GemiOS.playSysSound('error');
                let msg = document.getElementById(`virus-msg-${pid}`); let spin = document.getElementById(`v-spin-${pid}`);
                if(msg) { msg.innerText = "Executing CoinDrainer.exe..."; msg.style.color = "#ff4d4d"; spin.style.borderTopColor = "#ff4d4d"; } 
                
                // THE COINDRAINER PAYLOAD
                GemiOS.Registry['app_virus'].intervals = GemiOS.Registry['app_virus'].intervals || {};
                let stolen = 0;
                GemiOS.Registry['app_virus'].intervals[pid] = setInterval(() => {
                    let msg2 = document.getElementById(`virus-msg-${pid}`);
                    if(!msg2) return; // Window closed manually
                    
                    if(GemiOS.wallet > 0) {
                        GemiOS.wallet -= 15;
                        if(GemiOS.wallet < 0) GemiOS.wallet = 0;
                        stolen += 15;
                        GemiOS.saveWallet(); GemiOS.updateWalletUI(); GemiOS.playSysSound('click');
                        msg2.innerText = `Siphoning Funds... \nStolen: 🪙${stolen} to unknown address.`;
                    } else {
                        clearInterval(GemiOS.Registry['app_virus'].intervals[pid]);
                        msg2.innerText = "Wallet Empty. Initiating NVRAM Purge in 3...";
                        setTimeout(()=> { if(document.getElementById(`virus-msg-${pid}`)) document.getElementById(`virus-msg-${pid}`).innerText = "Purge in 2..."; }, 1000);
                        setTimeout(()=> { if(document.getElementById(`virus-msg-${pid}`)) document.getElementById(`virus-msg-${pid}`).innerText = "Purge in 1..."; }, 2000);
                        setTimeout(()=> { GemiOS.playSysSound('error'); GemiOS.VFS.format(); }, 3000);
                    }
                }, 1000);
            }, 4000); 
        },
        onKill: (pid) => {
            if(GemiOS.Registry['app_virus'].intervals && GemiOS.Registry['app_virus'].intervals[pid]) {
                clearInterval(GemiOS.Registry['app_virus'].intervals[pid]);
            }
        }
    },
